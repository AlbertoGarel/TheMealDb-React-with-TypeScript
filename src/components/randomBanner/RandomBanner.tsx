...
export default function RandomBanner({
  ...
  const paramURI = meal?.strMeal ? meal?.strMeal.split(' ').join('-') : undefined
  console.log("meal", paramURI);
      ...
      {meal && (
        <div id="random-banner">
          ...
          <div id="content" style={{textAlign: isSmall ? 'center' : 'left'  }}>
            <h2>{meal?.strMeal}</h2>
            {isSmall && <hr/>}
            <ul style={{width: isSmall ? '100%' : '50%', marginLeft: isSmall ? 0 : "20px"}}>
              <li>
                <h3>Categories</h3>
            ...
            <button type="button" className="btn btn1" onClick={() => navigate(`meal/${paramURI}`, { replace: true, state : {mealName: meal?.strMeal, item: meal} })}>Go to</button>
          </div>
        </div>
      )}
    </section>
